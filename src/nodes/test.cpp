...
int main(int argc, char** argv) {
  if (argc > 2) {
    ...
    int min_inliers = 3;
    if (argc > 5) min_inliers = atoi(argv[5]);

    int plot_mode = 0;
    if (argc > 6) plot_mode = atoi(argv[6]);
    ...
    base_tf << 0, 0.445417, 0.895323, 0.215, 1, 0, 0, -0.015, 0, 0.895323,
        -0.445417, 0.23, 0, 0, 0, 1;
    ...
    //    loc.localizeHands(file_name_left, file_name_right, indices,
    //    svm_file_name);

    //    // test with randomly sampled indices
    std::vector<GraspHypothesis> hands =
        loc.localizeHands(file_name_left, file_name_right);
    std::vector<GraspHypothesis> antipodal_hands =
        loc.predictAntipodalHands(hands, svm_file_name);
    std::vector<Handle> handles =
        loc.findHandles(antipodal_hands, min_inliers, 0.005);

    return 0;
  }

  std::cout << "No PCD filename given!\n";
  std::cout << "Usage: test_svm pcd_filepath svm_filepath [num_samples] "
               "[num_threads] [min_handle_inliers] [plot_mode]\n";
  ...
  std::cout
      << "  num_samples: number of samples to draw from the point cloud\n";
