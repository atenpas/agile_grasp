...
int main(int argc, char** argv) {
  if (argc > 3) {
    ...
    if (argc > 4) plots_hands = atoi(argv[4]);
    ...
    base_tf << 0, 0.445417, 0.895323, 0.21, 1, 0, 0, -0.02, 0, 0.895323,
        -0.445417, 0.24, 0, 0, 0, 1;

    sqrt_tf << 0.9366, -0.0162, 0.3500, -0.2863, 0.0151, 0.9999, 0.0058, 0.0058,
    ...
    // learn.trainBalanced(hand_list, hand_list_sizes, svm_file_name, cam_pos,
    // max_positives);
    learn.train(hand_list, hand_list_sizes, svm_file_name, cam_pos,
                max_positives);
  ...
  std::cout << "Usage: train_svm num_files pcd_directory svm_filename "
               "[plots_hands] [num_samples] [num_threads]\n";
  std::cout << "Train an SVM to localize grasps in point clouds.\n\n";
  std::cout << "The standard way is to create a directory that contains *.pcd "
               "files for "
            << "training. Each file needs to be called obji.pcd where i goes "
               "from 0 to <num_files>. "
            << "<pcd_directory> is the location and the root name of the "
               "files, for example, "
            << "/home/userA/data/obj.\n";
  std::cout
      << "Alternatively, if <num_files> is set to 0, there needs to be a "
         "files.txt "
      << "within pcd_directory that lists all filenames that should be used "
         "for training, and "
