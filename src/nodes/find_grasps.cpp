...
const std::string CLOUD_TYPES[2] = {"sensor_msgs/PointCloud2",
                                    "grasp_affordances/CloudSized"};
...
int main(int argc, char** argv) {
  ...
  node.param("hand_outer_diameter", params.hand_outer_diameter_,
             HAND_OUTER_DIAMETER);
  ...
  if (camera_pose.size() > 0) {
    ...
    for (int i = 0; i < R.rows(); i++)
      R.row(i) << camera_pose[i * R.cols()], camera_pose[i * R.cols() + 1],
          camera_pose[i * R.cols() + 2], camera_pose[i * R.cols() + 3];
    params.cam_tf_left_ = R;
    std::cout << "  camera pose:\n" << R << std::endl;
  }

  Eigen::VectorXd ws(6);
  ws << workspace[0], workspace[1], workspace[2], workspace[3], workspace[4],
      workspace[5];
