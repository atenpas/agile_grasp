...
Quadric::Quadric(
    const std::vector<Eigen::Matrix4d,
                      Eigen::aligned_allocator<Eigen::Matrix4d> >& T_cams,
    const PointCloud::Ptr& input, const Eigen::Vector3d& sample,
...
void Quadric::fitQuadric(const std::vector<int>& indices) {
  ...
  for (int i = 0; i < n; i++) {
    if (isnan(input_->points[indices[i]].x)) continue;
  ...
  N(8, 8) = n;
  // reflect upper triangular part in lower triangular part
  N.triangularView<Eigen::StrictlyLower>() =
      N.triangularView<Eigen::StrictlyUpper>().transpose();
  //~ std::cout<<"M:\n"<<M<<std::endl;
  //~ std::cout<<"N:\n"<<N<<std::endl;
...
void Quadric::findTaubinNormalAxis(const std::vector<int>& indices,
  ...
  if (!is_deterministic_) {
    ...
    if (indices.size() > num_samples) {
      for (int t = 0; t < num_samples; t++) {
        ...
        samples.col(t) << input_->points[indices[r]].x,
            input_->points[indices[r]].y, input_->points[indices[r]].z;
      ...
      for (int t = 0; t < indices.size(); t++) {
        samples.col(t) << input_->points[indices[t]].x,
            input_->points[indices[t]].y, input_->points[indices[t]].z;
      }
    }
  } else {
    samples_cam_source = cam_source;
  ...
  for (int cami = 0; cami < samples_cam_source.size(); cami++) {
    //    std::cout << "cami: " << cami << ", samples_cam_source(cami): " <<
    //    samples_cam_source(cami) << std::endl;
    if (samples_cam_source(cami) == 0)
      num_source(0)++;
    else if (samples_cam_source(cami) == 1)
      num_source(1)++;
  }
  num_source.maxCoeff(&majority_cam_source_);
  //  std::cout << "samples_cam_source.size(): " << samples_cam_source.size() <<
  //  ", num_source: " << num_source.transpose() << ", majority_cam_source_: "
  ...
  //~ std::cout<<"2.0*a*samples_near_surf.row(0) + d*samples_near_surf.row(1) +
  //f*samples_near_surf.row(2):\n"<<2.0*a*samples_near_surf.row(0) +
  //d*samples_near_surf.row(1) + f*samples_near_surf.row(2)<<"\n";
  //~ std::cout<<"2.0*a*samples_near_surf.row(0) + d*samples_near_surf.row(1) +
  //f*samples_near_surf.row(2).array():\n"<<(2.0*a*samples_near_surf.row(0) +
  //d*samples_near_surf.row(1) + f*samples_near_surf.row(2)).array()<<"\n";
  //~ std::cout<<"2.0*a*samples_near_surf.row(0) + d*samples_near_surf.row(1) +
  //f*samples_near_surf.row(2) + g:\n"<<(2.0*a*samples_near_surf.row(0) +
  //d*samples_near_surf.row(1) + f*samples_near_surf.row(2)).array() + g<<"\n";
  //~ std::cout<<"g: "<<g<<std::endl;

  // calculate normals at each of these points
  Eigen::MatrixXd fx =
      (2.0 * a * samples.row(0) + d * samples.row(1) + f * samples.row(2))
          .array() +
      g;
  //~ std::cout<<"fx:\n"<<fx<<std::endl;
  Eigen::MatrixXd fy =
      (2.0 * b * samples.row(1) + d * samples.row(0) + e * samples.row(2))
          .array() +
      h;
  //~ std::cout<<"fy:\n"<<fy<<std::endl;
  Eigen::MatrixXd fz =
      (2.0 * c * samples.row(2) + e * samples.row(1) + f * samples.row(0))
          .array() +
      i;
  //~ std::cout<<"fz:\n"<<fz<<std::endl;
  Eigen::MatrixXd normals(3, samples.cols());
  normals << fx, fy, fz;
  Eigen::MatrixXd gradient_magnitude =
      ((normals.cwiseProduct(normals)).colwise().sum()).cwiseSqrt();
...
void Quadric::findAverageNormalAxis(const Eigen::MatrixXd& normals) {
  ...
  std::sort(sorted_eigen_values.data(),
            sorted_eigen_values.data() + sorted_eigen_values.size());
  //~ std::cout<<"eigen_values:\n"<<eigen_values<<std::endl;
  ...
  (normals.transpose() * normals)
      ...
      .sum()
      .maxCoeff(&max_index);
  Eigen::Vector3d normpartial =
      (Eigen::MatrixXd::Identity(3, 3) -
       curvature_axis_ * curvature_axis_.transpose()) *
      normals.col(max_index);
  ...
  Eigen::Vector3d source_to_sample =
      sample_ - cam_origins_.col(majority_cam_source_);
  //  std::cout << source_to_sample.transpose() << std::endl;
  //  std::cout << "majority_cam_source: " << majority_cam_source_ << std::endl;
  //  std::cout << cam_origins.col(majority_cam_source).transpose() <<
  //  std::endl;
...
bool Quadric::solveGeneralizedEigenProblem(const Eigen::MatrixXd& A,
  ...
  if (B.cols() != N || A.rows() != N || B.rows() != N) return false;
  ...
  dggev_("N", "V", &N, A.data(), &LDA, B.data(), &LDB, alphar, alphai, beta, 0,
         &LDV, v.data(), &LDV, &WORKDUMMY, &LWORK, &INFO);

  LWORK = int(WORKDUMMY) + 32;
  Eigen::VectorXd WORK(LWORK);

  dggev_("N", "V", &N, A.data(), &LDA, B.data(), &LDB, alphar, alphai, beta, 0,
         &LDV, v.data(), &LDV, WORK.data(), &LWORK, &INFO);
...
void Quadric::plotAxes(void* viewer_void, int id) const {
  ...
  viewer->addLine<pcl::PointXYZ>(
      p, q, 0, 0, 255,
      "curvature_axis_" + boost::lexical_cast<std::string>(id));
  viewer->addLine<pcl::PointXYZ>(
      p, r, 255, 0, 0, "normal_axis_" + boost::lexical_cast<std::string>(id));
}
