...
std::vector<Handle> HandleSearch::findHandles(
  ...
  for (int i = 0; i < hand_list.size(); i++) {
    if (reduced_hand_list[i].getGraspWidth() == -1) continue;
    ...
    for (int j = 0; j < hand_list.size(); j++) {
      if (reduced_hand_list[j].getGraspWidth() == -1) continue;
      ...
      double dist_from_line =
          ((Eigen::Matrix3d::Identity(3, 3) - iaxis * iaxis.transpose()) *
           (jpt - ipt))
              .norm();
      double dist_along_line = iaxis.transpose() * (jpt - ipt);
      Eigen::Vector2d angle_axis;
      angle_axis << safeAcos(iaxis.transpose() * jaxis),
          M_PI - safeAcos(iaxis.transpose() * jaxis);
      double dist_angle_axis = angle_axis.minCoeff();
      double dist_from_normal = safeAcos(inormal.transpose() * jnormal);
      if (dist_from_line < 0.01 && dist_angle_axis < 0.34 &&
          dist_from_normal < 0.34) {
    ...
    if (inliers_list.size() < min_inliers) continue;
    ...
    for (int k = 0; k < inliers_list.size(); k++) {
      ...
      if (inliers_list[k](1) < min_dist) min_dist = inliers_list[k](1);
      if (inliers_list[k](1) > max_dist) max_dist = inliers_list[k](1);
    }
