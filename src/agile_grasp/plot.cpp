...
void Plot::plotFingers(const std::vector<Handle>& handle_list,
                       const PointCloud::Ptr& cloud, std::string str,
  ...
  for (int i = 0; i < handle_list.size(); i++) {
    ...
    GraspHypothesis center_hand(
        handle_list[i].getAxis(), handle_list[i].getApproach(),
        handle_list[i].getBinormal(), handle_list[i].getCenter(),
        handle_list[i].getCenter(), 0.0, Eigen::Matrix3Xd::Zero(3, 0),
  ...
  center_cloud_fingers =
      createFingersCloud(center_hand_list, outer_diameter, hand_depth);
...
PointCloud::Ptr Plot::createFingersCloud(
    const std::vector<GraspHypothesis>& hand_list, double outer_diameter,
    double hand_depth) {
  PointCloud::Ptr cloud_fingers(new PointCloud);

  for (int i = 0; i < hand_list.size(); i++) {
    Eigen::Vector3d bottom =
        hand_list[i].getGraspBottom() - hand_depth * hand_list[i].getApproach();
...
void Plot::plotHands(const std::vector<GraspHypothesis>& hand_list,
  ...
  PointCloudNormal::Ptr hands_cloud =
      createNormalsCloud(hand_list, false, false);
  PointCloudNormal::Ptr antipodal_hands_cloud =
      createNormalsCloud(antipodal_hand_list, true, false);
  plotHandsHelper(hands_cloud, antipodal_hands_cloud, cloud, str,
                  use_grasp_bottom);
}
...
void Plot::plotHands(const std::vector<GraspHypothesis>& hand_list,
  ...
  PointCloudNormal::Ptr hands_cloud =
      createNormalsCloud(hand_list, false, false);
  PointCloudNormal::Ptr antipodal_hands_cloud =
      createNormalsCloud(hand_list, true, false);
  plotHandsHelper(hands_cloud, antipodal_hands_cloud, cloud, str,
                  use_grasp_bottom);
}
...
void Plot::plotSamples(const std::vector<int>& index_list,
  ...
  boost::shared_ptr<pcl::visualization::PCLVisualizer> viewer =
      createViewer("Samples");
  addCloudToViewer(viewer, cloud, "cloud");
  addCloudToViewer(viewer, samples_cloud, "samples");
  viewer->setPointCloudRenderingProperties(
      pcl::visualization::PCL_VISUALIZER_COLOR, 1.0, 0.0, 0.0, "samples");
...
PointCloudNormal::Ptr Plot::createNormalsCloud(
    const std::vector<GraspHypothesis>& hand_list, bool plots_only_antipodal,
  ...
  for (int i = 0; i < hand_list.size(); i++) {
    ...
    if (!plots_only_antipodal ||
        (plots_only_antipodal && hand_list[i].isFullAntipodal())) {
      pcl::PointNormal p;
...
void Plot::addCloudNormalsToViewer(
    boost::shared_ptr<pcl::visualization::PCLVisualizer>& viewer,
    const PointCloudNormal::Ptr& cloud, double line_width, double* color_cloud,
    double* color_normals, const std::string& cloud_name,
...
void Plot::plotHandsHelper(const PointCloudNormal::Ptr& hands_cloud,
                           ...
                           const PointCloud::Ptr& cloud, std::string str,
                           bool use_grasp_bottom) {
  std::cout << "Drawing " << hands_cloud->size() << " grasps of which "
            << antipodal_hands_cloud->size() << " are antipodal grasps.\n";
  ...
  addCloudNormalsToViewer(viewer, hands_cloud, 2, green, cyan,
                          std::string("hands"), std::string("approaches"));

  if (antipodal_hands_cloud->size() > 0) {
    double red[3] = {1.0, 0.0, 0.0};
    addCloudNormalsToViewer(viewer, antipodal_hands_cloud, 2, green, red,
                            std::string("antipodal_hands"),
...
void Plot::runViewer(
  ...
  //	std::cout << "Cam: " << std::endl << " - pos: (" << cam[0].pos[0] << ",
  //" << cam[0].pos[1] << ", "
  //			<< cam[0].pos[2] << ")" << std::endl << " - view: (" <<
  //cam[0].view[0] << ", " << cam[0].view[1] << ", "
  //			<< cam[0].view[2] << ")" << std::endl << " - focal: (" <<
  //cam[0].focal[0] << ", " << cam[0].focal[1]
  //			<< ", " << cam[0].focal[2] << ")" << std::endl;
}

boost::shared_ptr<pcl::visualization::PCLVisualizer> Plot::createViewer(
    std::string title) {
  boost::shared_ptr<pcl::visualization::PCLVisualizer> viewer(
...
void Plot::createVisualPublishers(ros::NodeHandle& node,
  ...
  hypotheses_pub_ = node.advertise<visualization_msgs::MarkerArray>(
      "grasp_hypotheses_visual", 10);
  antipodal_pub_ = node.advertise<visualization_msgs::MarkerArray>(
      "antipodal_grasps_visual", 10);
  handles_pub_ =
      node.advertise<visualization_msgs::MarkerArray>("handles_visual", 10);
...
void Plot::plotGraspsRviz(const std::vector<GraspHypothesis>& hand_list,
  ...
  for (int i = 0; i < hand_list.size(); i++) {
    ...
    visualization_msgs::Marker marker = createApproachMarker(
        frame, position, hand_list[i].getApproach(), i, color, 0.4, 0.004);
...
void Plot::plotHandlesRviz(const std::vector<Handle>& handle_list,
  ...
  for (int i = 0; i < handle_list.size(); i++) {
    ...
    visualization_msgs::Marker marker = createApproachMarker(
        frame, position, handle_list[i].getApproach(), i, green, 0.6, 0.008);
    marker.ns = "handles";
    marker_array.markers[i] = marker;
  }
...
void Plot::plotHandles(const std::vector<Handle>& handle_list,
                       const PointCloud::Ptr& cloud, std::string str) {
  double colors[10][3] = {{0, 0.4470, 0.7410},      {0.8500, 0.3250, 0.0980},
  ...
  pcl::PointCloud<pcl::PointNormal>::Ptr handle_cloud(
  ...
  for (int i = 0; i < handle_list.size(); i++) {
    ...
    const std::vector<int>& inliers = handle_list[i].getInliers();
    const std::vector<GraspHypothesis>& hand_list =
        handle_list[i].getHandList();
    pcl::PointCloud<pcl::PointNormal>::Ptr axis_cloud(
        new pcl::PointCloud<pcl::PointNormal>);
  ...
  for (int i = 0; i < clouds.size(); i++) {
    ...
    viewer->setPointCloudRenderingProperties(
        pcl::visualization::PCL_VISUALIZER_COLOR, colors[ci][0], colors[ci][1],
        colors[ci][2], name);
    ...
    viewer->setPointCloudRenderingProperties(
        pcl::visualization::PCL_VISUALIZER_LINE_WIDTH, 2, name2);
    viewer->setPointCloudRenderingProperties(
        pcl::visualization::PCL_VISUALIZER_COLOR, colors[ci][0], colors[ci][1],
        colors[ci][2], name2);
  }
  ...
  viewer->setPointCloudRenderingProperties(
      pcl::visualization::PCL_VISUALIZER_COLOR, 0, 0, 0, "handles");
  viewer->addPointCloudNormals<pcl::PointNormal>(handle_cloud, 1, 0.08,
                                                 "approach");
  viewer->setPointCloudRenderingProperties(
      pcl::visualization::PCL_VISUALIZER_LINE_WIDTH, 4, "approach");
  viewer->setPointCloudRenderingProperties(
      pcl::visualization::PCL_VISUALIZER_COLOR, 0, 0, 0, "approach");
...
visualization_msgs::Marker Plot::createApproachMarker(
    const std::string& frame, const geometry_msgs::Point& center,
    const Eigen::Vector3d& approach, int id, const double* color, double alpha,
    double diam) {
