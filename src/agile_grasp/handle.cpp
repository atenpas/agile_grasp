...
void Handle::setAxis() {
  ...
  Eigen::EigenSolver<Eigen::Matrix3d> eigen_solver(axis_mat *
                                                   axis_mat.transpose());
...
void Handle::setDistAlongHandle() {
  ...
  for (int i = 0; i < inliers_.size(); i++) {
    dist_along_handle_(i) =
        axis_.transpose() * hand_list_[inliers_[i]].getGraspBottom();
  }
}

void Handle::setGraspVariables() {
  double center_dist =
      (dist_along_handle_.maxCoeff() + dist_along_handle_.minCoeff()) / 2.0;
  ...
  //	std::cout << "hands_center_: " << hands_center_.transpose() <<
  //std::endl;
  //	std::cout << std::endl;
}
