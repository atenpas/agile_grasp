...
RotatingHand::RotatingHand(const Eigen::VectorXd& camera_origin_left,
    ...
    : finger_hand_(finger_hand),
      tolerant_antipodal_(tolerant_antipodal),
  ...
  Eigen::VectorXd angles =
      Eigen::VectorXd::LinSpaced(num_orientations + 1, -1.0 * M_PI, M_PI);
...
void RotatingHand::transformPoints(const Eigen::Matrix3Xd& points,
  ...
  //  std::endl;
  //  std::cout << " frame_: " << frame_.rows() << " x " << frame_.cols() <<
  //  std::endl;
  //  std::cout << frame_ << std::endl;
  //  std::cout << " points_: " << points_.rows() << " x " << points_.cols() <<
  //  std::endl;
  ...
  int k = 0;
  //  std::cout << "h.pts: " << points_.cols() << ", -1.0 * hand_height: " <<
  //  -1.0 * hand_height << "\n";
...
std::vector<GraspHypothesis> RotatingHand::evaluateHand(
    double init_bite, const Eigen::Vector3d& sample, bool use_antipodal) {
  ...
  for (int i = 0; i < n; i++) {
    ...
    Eigen::Matrix3d rot;
    rot << cos(angles_(i)), -1.0 * sin(angles_(i)), 0.0, sin(angles_(i)),
        cos(angles_(i)), 0.0, 0.0, 0.0, 1.0;
    ...
    if (finger_hand_.getHand().sum() > 0) {
      ...
      for (int j = 0; j < points_rot.cols(); j++) {
        if (points_rot(1, j) <
            finger_hand_.getBackOfHand() + finger_hand_.getHandDepth())
      ...
      for (int j = 0; j < points_in_box_indices.size(); j++) {
        points_in_box.col(j) =
            points_rot.col(points_in_box_indices[j]) - surface;
      ...
      GraspHypothesis grasp(hand_axis_, approach, binormal, bottom, surface,
                            finger_hand_.getGraspWidth(), points_in_box,
                            indices_cam1, indices_cam2, cam_source_);

      if (use_antipodal)  // if we need to evaluate whether the grasp is
                          // antipodal
      {
