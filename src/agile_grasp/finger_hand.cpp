...
void FingerHand::evaluateFingers(double bite) {
  ...
  //  std::cout << "     -A- " << " " << cropped_indices.size() << " " <<
  //  points_.cols() << "\n";
  ...
  //  std::cout << "m: " << m << ", finger_spacing_: " <<
  //  finger_spacing_.transpose() << " " << finger_spacing_.size() << std::endl;
  for (int i = 0; i < m; i++) {
    ...
    //    std::cout << "i: " << i << ", numInGap: " << num_in_gap << "
    //    finger_width_: " << finger_width_ << " finger_spacing_: " <<
    //    finger_spacing_(i) << std::endl;
    ...
    if (num_in_gap == 0) {
      ...
      if (i <= m / 2) {
        ...
        sum =
            (cropped_points.row(0).array() > finger_spacing_(i) + finger_width_)
                .count();
      } else {
        //        std::cout << " i > m/2\n";
      ...
      //      cropped_points.array(): " << cropped_points.array().rows() << " x
      //      " << cropped_points.array().cols() << std::endl;
...
void FingerHand::evaluateGraspParameters(double bite) {
  ...
  //      + (finger_spacing_.block(0, 0, 1, hand_.cols()).array() *
  //      hand_.cast<double>()).sum() / hand_.sum();
  //  std::cout << "hand_: " << hand_ << ", hand_.sum(): " << hand_.sum() <<
  //  std::endl;
  //  std::cout << "finger_spacing_: " << finger_spacing_.block(0, 0, 1,
  //  hand_.cols()).array() << std::endl;
  //  std::cout << "hand_.cast<double>(): " << hand_.cast<double>() <<
  //  std::endl;
  //  std::cout << finger_spacing_.block(0, 0, 1, hand_.cols()).array() *
  //  hand_.cast<double>() << std::endl;
  //  std::cout << (finger_spacing_.block(0, 0, 1, hand_.cols()).array() *
  //  hand_.cast<double>()).sum() << std::endl;
  ...
  //	std::cout << "fs_sum: " << fs_sum << ", hor_pos: " << hor_pos << ",
  //hand_: "
  //			<< hand_ << "\n";
  ...
  // calculate handWidth. First find eroded hand. Then take points contained
  // within two fingers of that hand.
  std::vector<int> hand_idx;
  for (int i = 0; i < hand_.cols(); i++) {
    if (hand_(i) == true) hand_idx.push_back(i);
  }
  ...
  double max = -100000.0;
  double min = 100000.0;
  //  std::cout << " EGP(2) left_finger_pos: " << left_finger_pos << "
  //  right_finger_pos: " << right_finger_pos << std::endl;
  for (int i = 0; i < points_.cols(); i++) {
    if (points_(1, i) < bite && points_(0, i) > left_finger_pos &&
        points_(0, i) < right_finger_pos) {
      if (points_(0, i) < min) min = points_(0, i);
      if (points_(0, i) > max) max = points_(0, i);
    }
  }
...
void FingerHand::deepenHand(double init_deepness, double max_deepness) {
  ...
  //  std::cout << "d = " << init_deepness + deepen_step_size << ",
  //  max_deepness: " << max_deepness << std::endl;
  for (double d = init_deepness + deepen_step_size; d <= max_deepness;
       d += deepen_step_size) {
    //    std::cout << "d(): " << d << std::endl;
    new_hand.evaluateFingers(d);
    new_hand.evaluateHand();
    //		std::cout << "d: " << d << ", new_hand.hand_: " <<
    //new_hand.getHand()
    //				<< std::endl;
    //		std::cout << "hand_eroded: " << hand_eroded << std::endl;
    //    std::cout << " => last_new_hand.hand_: " << last_new_hand.getHand() <<
    //    std::endl;
    //     if ((new_hand.hand_ && hand_eroded).count() == 0) // hand is so deep
    //     that it does not exist any longer
    //       break;
    bool is_too_deep = true;
    for (int i = 0; i < hand_eroded.cols(); i++) {
      if (new_hand.hand_(i) == 1 &&
          hand_eroded(i) ==
              1)  // hand is not so deep that it does not exist any longer
      {
        is_too_deep = false;
        break;
      }
    }
    if (is_too_deep) break;
    last_new_hand = new_hand;
    //    std::cout << "d[]: " << d << ", last_new_hand.hand: " <<
    //    last_new_hand.getHand() << ", new_hand.hand: " << new_hand.getHand()
    //    << std::endl;
  }
  *this = last_new_hand;  // recover most deep hand
  hand_ = hand_eroded;
  //	std::cout << "deepenHand done. hand_: " << hand_ << ", fingers_: " <<
  //fingers_
  //			<< std::endl;
}
