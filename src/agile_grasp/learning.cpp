...
void Learning::trainBalanced(const std::vector<GraspHypothesis>& hands_list,
                             ...
                             const Eigen::Matrix3Xd& cam_pos, int max_positive,
                             bool is_plotting) {
  ...
  for (int i = 0; i < hands_list.size(); i++) {
    ...
    if (i == sizes[k]) {
      if (positives_sub.size() <= max_positive) {
        positives.insert(positives.end(), positives_sub.begin(),
                         positives_sub.end());
  ...
  indices_selected.insert(indices_selected.end(), positives.begin(),
                          positives.end());
  std::set<int> indices;
  ...
  std::cout << "indices_selected.size: " << indices_selected.size()
            << std::endl;
  ...
  std::cout << "Converting " << instances.size()
            << " training examples (grasps) to images\n";
    ...
    if (i == sizes[k]) {
      ...
      if (positives.size() <= max_positive) {
        for (int j = 0; j < positives.size(); j++) {
          instances.push_back(
              createInstance(hands_list[positives[j]], cam_pos));

          // add instances for simulated left and right camera
          instances.push_back(
              createInstance(hands_list[positives[j]], cam_pos, 0));
          instances.push_back(
              createInstance(hands_list[positives[j]], cam_pos, 1));
        }
      } else  // select <max_positive> positive examples randomly
      {
        std::set<int> indices;
        ...
        for (std::set<int>::iterator it = indices.begin(); it != indices.end();
          ...
          std::cout << " " << *it;
          instances.push_back(
              createInstance(hands_list[positives[*it]], cam_pos));

          // add instances for simulated left and right camera
          instances.push_back(
              createInstance(hands_list[positives[*it]], cam_pos, 0));
          instances.push_back(
              createInstance(hands_list[positives[*it]], cam_pos, 1));
        }
...
std::vector<GraspHypothesis> Learning::classify(
...
void Learning::convertData(const std::vector<Instance>& instances,
  ...
  for (int i = 0; i < instances.size(); i++) {
    ...
    std::vector<float> descriptors;
    std::vector<cv::Point> locations;
    //		std::cout << "HOG descriptor size is " << hog.getDescriptorSize() <<
    //std::endl;
    hog.compute(image, descriptors, cv::Size(32, 32), cv::Size(0, 0),
                locations);
    //		std::cout << "HOG descriptor size is " << hog.getDescriptorSize() <<
  ...
  //svm.save(file_name.c_str());
  std::cout << "# training examples: " << features.rows
...
cv::Mat Learning::convertToImage(const Instance& ins) {
  ...
  double cell_size = (HORIZONTAL_LIMITS[1] - HORIZONTAL_LIMITS[0]) /
                     (double)num_horizontal_cells_;
  ...
  if (ins.binormal.dot(ins.source_to_center) > 0)
    horizontal_cells = floorVector(
        (ins.pts.row(0).array() - HORIZONTAL_LIMITS[0]) / cell_size);
  else
    horizontal_cells = floorVector(
        (-ins.pts.row(0).array() - HORIZONTAL_LIMITS[0]) / cell_size);

  vertical_cells =
      floorVector((ins.pts.row(1).array() - VERTICAL_LIMITS[0]) / cell_size);
  ...
  for (std::set<Eigen::Vector2i, UniqueVectorComparator>::iterator it =
           cells.begin();
       it != cells.end(); it++) {
    Eigen::Vector2i c = *it;
...
Eigen::VectorXi Learning::floorVector(const Eigen::VectorXd& a) {
  ...
  for (int i = 0; i < b.size(); i++) b(i) = floor(a(i));
  return b;
}
...
Learning::Instance Learning::createInstance(const GraspHypothesis& h,
  ...
  if (cam == -1) {
    ...
    const std::vector<int>& indices_cam =
        (cam == 0) ? h.getIndicesPointsForLearningCam1()
                   : h.getIndicesPointsForLearningCam2();
