...
std::vector<GraspHypothesis> HandSearch::findHands(
    const PointCloud::Ptr cloud, const Eigen::VectorXi& pts_cam_source,
    const std::vector<int>& indices, const PointCloud::Ptr cloud_plot,
    bool calculates_antipodal, bool uses_clustering) {
  ...
  if (calculates_antipodal) {
    ...
    for (int i = 0; i < indices_cloud.size(); i++) indices_cloud[i] = i;
  ...
  if (plots_samples_) plot_.plotSamples(indices_rand, cloud);

  // find quadrics
  std::cout << "Estimating local axes ...\n";
  std::vector<Quadric> quadric_list =
      findQuadrics(cloud, pts_cam_source, kdtree, indices_rand);
  if (plots_local_axes_) plot_.plotLocalAxes(quadric_list, cloud_plot);

  // find hands
  std::cout << "Finding hand poses ...\n";
  std::vector<GraspHypothesis> hand_list =
      findHands(cloud, pts_cam_source, quadric_list, hands_cam_source, kdtree);

  return hand_list;
}

std::vector<Quadric> HandSearch::findQuadrics(
    const PointCloud::Ptr cloud, const Eigen::VectorXi& pts_cam_source,
...
#pragma omp parallel for private(nn_indices, nn_dists) num_threads(num_threads_)
  ...
  for (int i = 0; i < indices.size(); i++) {
    ...
    //    std::cout << "i: " << i << ", index: " << indices[i] << ", sample: "
    //    << sample << std::endl;

    if (kdtree.radiusSearch(sample, nn_radius_taubin_, nn_indices, nn_dists) >
        0) {
      ...
      Quadric q(T_cams, cloud, sample_eigen,
  ...
  std::cout << "Fitted " << quadric_list.size() << " quadrics in " << t2 - t1
            << " sec.\n";
...
std::vector<GraspHypothesis> HandSearch::findHands(
  ...
  std::vector<std::vector<GraspHypothesis> > grasp_lists(
      quadric_list.size(), std::vector<GraspHypothesis>(0));

#ifdef _OPENMP  // parallelization using OpenMP
#pragma omp parallel for private(nn_indices, nn_dists, nn_normals,     \
                                 nn_cam_source, centered_neighborhood) \
  ...
  for (std::size_t i = 0; i < quadric_list.size(); i++) {
    ...
    if (kdtree.radiusSearch(sample, nn_radius_hands_, nn_indices, nn_dists) >
        0) {
      ...
      for (int j = 0; j < nn_indices.size(); j++) {
        nn_cam_source(j) = pts_cam_source(nn_indices[j]);
        centered_neighborhood.col(j) =
            (cloud->points[nn_indices[j]].getVector3fMap() -
             sample.getVector3fMap())
                .cast<double>();
      ...
      RotatingHand rotating_hand(cam_tf_left_.block<3, 1>(0, 3) - sample_eig,
                                 cam_tf_right_.block<3, 1>(0, 3) - sample_eig,
                                 finger_hand, tolerant_antipodal_,
                                 hands_cam_source(i));
      const Quadric& q = quadric_list[i];
      double time_tf1 = omp_get_wtime();
      rotating_hand.transformPoints(centered_neighborhood, q.getNormal(),
                                    q.getCurvatureAxis(), nn_normals,
                                    nn_cam_source, hand_height_);
      time_tf += omp_get_wtime() - time_tf1;
      double time_eval1 = omp_get_wtime();
      std::vector<GraspHypothesis> grasps =
          rotating_hand.evaluateHand(init_bite_, sample_eig, true);
  ...
  std::cout << " avg time for transforming point neighborhood: " << time_tf
            << " sec.\n";
  std::cout << " avg time for NN search: " << time_nn << " sec.\n";
  std::cout << " avg time for rotating_hand.evaluate(): " << time_eval_hand
            << " sec.\n";
  ...
  for (std::size_t i = 0; i < grasp_lists.size(); i++) {
    ...
    if (grasp_lists[i].size() > 0)
      grasp_list.insert(grasp_list.end(), grasp_lists[i].begin(),
                        grasp_lists[i].end());
  }

  double t2 = omp_get_wtime();
  std::cout << " Found " << grasp_list.size() << " robot hand poses in "
            << t2 - t1 << " sec.\n";
