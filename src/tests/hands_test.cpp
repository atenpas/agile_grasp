...
int main(int argc, char** argv) {
  ...
  if (pcl::io::loadPCDFile<pcl::PointXYZRGBA>(file, *cloud) ==
      -1)  //* load the file
  {
  ...
  pcl::KdTreeFLANN<pcl::PointXYZRGBA>::Ptr tree(
  ...
  if (cloud->isOrganized()) {
    ...
    organized_neighbor->radiusSearch(cloud->points[sample_index], taubin_radius,
                                     nn_indices, nn_dists);
  } else {
    tree->setInputCloud(cloud);
    tree->radiusSearch(cloud->points[sample_index], taubin_radius, nn_indices,
                       nn_dists);
  }
  ...
  T_base << 0, 0.445417, 0.895323, 0.22, 1, 0, 0, -0.02, 0, 0.895323, -0.445417,
      0.24, 0, 0, 0, 1;

  T_sqrt << 0.9366, -0.0162, 0.3500, -0.2863, 0.0151, 0.9999, 0.0058, 0.0058,
      -0.3501, -0.0002, 0.9367, 0.0554, 0, 0, 0, 1;
  ...
  Eigen::Vector3d sample =
      cloud->points[sample_index].getVector3fMap().cast<double>();
  ...
  // fit hand
  tree->radiusSearch(cloud->points[sample_index], hand_radius, nn_indices,
                     nn_dists);
  ...
  for (int j = 0; j < nn_indices.size(); j++) {
    ...
    centered_neighborhood.col(j) =
        cloud->points[nn_indices[j]].getVector3fMap().cast<double>() - sample;
  }
  ...
  RotatingHand rotating_hand(T_cams[0].block(0, 3, 3, 1) - sample,
                             T_cams[0].block(0, 3, 3, 1) - sample, finger_hand,
                             true, pts_cam_source(sample_index));
  rotating_hand.transformPoints(centered_neighborhood, quadric.getNormal(),
                                quadric.getCurvatureAxis(), nn_normals,
                                nn_cam_source, hand_height_);
  std::vector<GraspHypothesis> h =
